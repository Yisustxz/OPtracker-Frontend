...
export default function ProcedurePage() {
      ...
      prev.map((proc, index) => {
        if (procedureId === proc.id) {
          ...
          if (canToggle) {
            ...
            axios
              .patch(`http://localhost:3000/surgery/procedure/status`, {
  ...
  const handleCancelProcedure = async () => {
    try {
      const { title, date, patientId, nurseIds, doctorIds, procedureIds } =
        surgery; // Obtener datos de surgery
      ...
      <div className="flex flex-col items-center pt-20 px-5">
        <div className="w-full max-w-[960px]">
              ...
              <li key={procedure.id} className="flex items-center mb-8">
                <input
                  ...
                  onChange={() =>
                    procedure.completed
                      ? null
                      : toggleProcedure(id, procedure.id)
                  } // No permitir desmarcar si está completado
                  className="mr-2 border border-gray-300 bg-white"
                  disabled={
                    procedure.completed ||
                    (surgery && surgery.status === "CANCELLED")
                  } // Deshabilitar el checkbox si está completado o si la cirugía está cancelada
                />
                <span
                  className={
                    procedure.completed
          ...
          {surgery &&
            ...
            surgery.status !== "DONE" && ( // Mostrar botones solo si la cirugía no está cancelada
