...
export default function PatientTracker() {
  const initialCheckedItems = timelineSteps.map(step => step.time ? true : false);
  const [checkedItems, setCheckedItems] = useState(initialCheckedItems);

  const handleCheckboxChange = (index) => {
    if (timelineSteps[index].time && !checkedItems[index]) { // Solo permitir marcar si no está marcado
  ...
  return (
    <div className="patient-tracker">
      ...
      <main className="main-content" style={{ marginTop: '65px' }}>
        <div className="patient-id" style={{ textAlign: 'center', position: 'absolute', top: '1%', left: '50%', transform: 'translate(-50%, -50%)' }}>007</div>
        ...
        <section className="team-section">
          ...
          <div className="team-container">
            ...
            <div style={{ textAlign: 'right', marginTop: "30px", fontSize: '12px', display: 'flex', justifyContent: 'space-between', cursor: 'pointer' }}>
              <span></span>
              <span style={{ color: 'blue', transition: 'color 0.3s' }} onMouseEnter={(e) => e.target.style.color = 'darkblue'} onMouseLeave={(e) => e.target.style.color = 'blue'}>
                Ver más
              </span>
            </div>
          </div>
          ...
          <div className="timeline-container">
            {timelineSteps.map((step, index) => (
              <div key={index} className="timeline-step">
                ...
                <span>{step.title} - {step.time}</span>
              </div>
            ))}
          </div>
